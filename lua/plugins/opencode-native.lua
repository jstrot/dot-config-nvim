...
return {
  {
    ...
    opts = {
      ...
      keymap = {
        editor = {
          -- ['<leader>aI'] = false,
          ['<leader>aA'] = { 'select_agent', desc = "[A]gentic/Opencode: select [A]gent" },
          ['<leader>aC'] = { function() vim.api.nvim_command('edit ~/.config/opencode/opencode.jsonc') end, desc = "[A]gentic/Opencode: edit opencode [C]onfig" },
          ['<leader>aR'] = { 'rename_session', desc = '[A]gentic/Opencode: [R]ename opencode session', mode = 'n' }, -- Rename current session
          ['<leader>aT'] = { 'timeline', desc = '[A]gentic/Opencode: [T]imeline picker', mode = 'n' }, -- Display timeline picker to navigate/undo/redo/fork messages
          ['<leader>a[d'] = { 'diff_prev', desc = '[A]gentic/Opencode: previous diff', mode = 'n' }, -- Navigate to previous file diff
          ['<leader>a]d'] = { 'diff_next', desc = '[A]gentic/Opencode: next diff', mode = 'n' }, -- Navigate to next file diff
          ['<leader>ac'] = { 'diff_close', desc = '[A]gentic/Opencode: [C]lose diff view', mode = 'n' }, -- Close diff view tab and return to normal editing
          ['<leader>ad'] = { 'diff_open', desc = '[A]gentic/Opencode: open [D]iff view', mode = 'n' }, -- Opens a diff tab of a modified file since the last opencode prompt
          ['<leader>af'] = { 'toggle_focus', desc = '[A]gentic/Opencode: [F]ocus opencode/last window', mode = 'n' }, -- Toggle focus between opencode and last window
          -- ['<leader>ag'] = false,
          ['<leader>ai'] = { 'open_input', desc = '[A]gentic/Opencode: open [I]nput window', mode = 'n' }, -- Opens and focuses on input window on insert mode
          ['<leader>am'] = { 'configure_provider', desc = '[A]gentic/Opencode: configure provider/[M]odel', mode = 'n' }, -- Quick provider and model switch from predefined list
          ['<leader>an'] = { 'open_input_new_session', desc = '[A]gentic/Opencode: [N]ew opencode session', mode = 'n' }, -- Opens and focuses on input window on insert mode. Creates a new session
          ['<leader>ao'] = { 'open_output', desc = '[A]gentic/Opencode: open [O]utput window', mode = 'n' }, -- Opens and focuses on output window
          -- ['<leader>ap'] = false,
          ['<leader>apA'] = { 'permission_accept_all', desc = '[A]gentic/Opencode: [P]ermission [A]ccept all', mode = 'n' }, -- Accept all (for current tool)
          ['<leader>apa'] = { 'permission_accept', desc = '[A]gentic/Opencode: [P]ermission [A]ccept once', mode = 'n' }, -- Accept permission request once
          ['<leader>apd'] = { 'permission_deny', desc = '[A]gentic/Opencode: [P]ermission [D]eny', mode = 'n' }, -- Deny permission request once
          ['<leader>aq'] = { 'close', desc = '[A]gentic/Opencode: [Q]uit/close opencode', mode = 'n' }, -- Close UI windows
          ['<leader>arA'] = { 'diff_revert_all', desc = '[A]gentic/Opencode: [R]evert [A]ll changes from session', mode = 'n' }, -- Revert all file changes since the last opencode session
          ['<leader>arR'] = { 'diff_restore_snapshot_all', desc = '[A]gentic/Opencode: [R]estore snapshot [A]ll files', mode = 'n' }, -- Restore all files to a restore point
          ['<leader>arT'] = { 'diff_revert_this', desc = '[A]gentic/Opencode: [R]evert [T]his file from session', mode = 'n' }, -- Revert current file changes since the last opencode session
          ['<leader>ara'] = { 'diff_revert_all_last_prompt', desc = '[A]gentic/Opencode: [R]evert [A]ll changes from last prompt', mode = 'n' }, -- Revert all file changes since the last opencode prompt
          ['<leader>arr'] = { 'diff_restore_snapshot_file', desc = '[A]gentic/Opencode: [R]estore snapshot [F]ile', mode = 'n' }, -- Restore a file to a restore point
          ['<leader>art'] = { 'diff_revert_this_last_prompt', desc = '[A]gentic/Opencode: [R]evert [T]his file from last prompt', mode = 'n' }, -- Revert current file changes since the last opencode prompt
          ['<leader>as'] = { 'select_session', desc = '[A]gentic/Opencode: [S]elect opencode session', mode = 'n' }, -- Select and load a opencode session
          ['<leader>at'] = { 'toggle', desc = '[A]gentic/Opencode: [T]oggle opencode', mode = 'n' }, -- Open opencode. Close if opened
          ['<leader>av'] = { 'paste_image', desc = '[A]gentic/Opencode: paste image from clipboard', mode = 'n' }, -- Paste image from clipboard into current session
          ['<leader>ax'] = { 'swap_position', desc = '[A]gentic/Opencode: e[X]change pane position', mode = 'n' }, -- Swap Opencode pane left/right
          ['<leader>az'] = { 'toggle_zoom', desc = '[A]gentic/Opencode: [Z]oom opencode windows', mode = 'n' }, -- Zoom in/out on the Opencode windows
        },
        input_window = {
        ...
        output_window = {
          ...
          ['<esc>'] = false,
          ['<leader>aD'] = false,
          ['<leader>aDm'] = { 'debug_message', desc = '[A]gentic/Opencode: [D]ebug [M]essage', mode = 'n' }, -- Open raw message in new buffer for debugging
          ['<leader>aDo'] = { 'debug_output', desc = '[A]gentic/Opencode: [D]ebug [O]utput', mode = 'n' }, -- Open raw output in new buffer for debugging
          ['<leader>aDs'] = { 'debug_session', desc = '[A]gentic/Opencode: [D]ebug [S]ession', mode = 'n' }, -- Open raw session in new buffer for debugging
          ['<leader>aO'] = false,
